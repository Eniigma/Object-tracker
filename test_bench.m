...
% 3 frames, det 1,2 3,4 5,6
c = -0.2;
dres.c = [c c c c c c c c];
...
dat_in = zeros(25,3); %% each row represents an edge from node in column 1 to node in column 2 with cost in column 3.
k_dat = 0;
for i = 1:dnum     %% for each individual detection
  k_dat = k_dat+3;
  dat_in(k_dat-2,:) = [1      2*i     c_en      ];
  dat_in(k_dat-1,:) = [2*i    2*i+1   c*1e6];
...
k_dat = k_dat+1;
i=1,j=3;
dat_in(k_dat,:) = [2*i+1 2*j cij(i,j)];
% k_dat = k_dat+1;
% i=1,j=4;
% dat_in(k_dat,:) = [2*i+1 2*j cij(i,j)];
k_dat = k_dat+1;
i=1,j=5;
dat_in(k_dat,:) = [2*i+1 2*j cij1];
k_dat = k_dat+1;
i=2,j=3;
dat_in(k_dat,:) = [2*i+1 2*j cij(i,j)];
k_dat = k_dat+1;
...
while ub-lb > 1     %% bisection search for the optimum amount of flow. This can be implemented by Golden section search more efficiently.
  tr_num = round((lb+ub)/2);   % constraint : flow < no of detections
%   tr_num = 6;
