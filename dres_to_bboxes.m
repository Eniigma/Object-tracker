...
for i = 1:length(dres.x)
  bbox = [dres.x(i) dres.y(i) dres.x(i)+dres.w(i) dres.y(i)+dres.h(i) dres.id(i) dres.r(i)];
  bboxes(dres.fr(i)).bbox = [bboxes(dres.fr(i)).bbox; bbox];
  traj = dres.tr1(i).bbox;
  id = dres.id(i);
  for j=1:delta-1
    bbox1 = [traj(j+1,1:2) traj(j+1,1)+traj(j+1,3) traj(j+1,2)+traj(j+1,4) id dres.r(i)];
